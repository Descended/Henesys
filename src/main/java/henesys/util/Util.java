...
public class Util {
    ...
    /**
     * Returns a random selection from an array of integers.
     * This implementation uses ThreadLocalRandom, and is initialised with an internally generated seed that may not otherwise be modified.
     * This implementation should result in less overhead and contention, as compared to shared Random objects used in the default Swordie implementations.
     *
     * @author KOOKIIE
     * @param items Array of integers to select from
     * @return      Integer selected at random; defaults to 0 if nothing is selected
     */
    public static int getRandom(int[] items) {
        try {
            return items[ThreadLocalRandom.current().nextInt(items.length)];
        } catch (Exception e) {
            log.warn("An invalid array has been passed into getRandom().", e);
        }
        return 0;
    }

    /**
     * Returns a random integer from 0 to 100 (exclusive).
     * This implementation uses ThreadLocalRandom, and is initialised with an internally generated seed that may not otherwise be modified.
     * This implementation should result in less overhead and contention, as compared to shared Random objects used in the default Swordie implementations.
     *
     * @author KOOKIIE
