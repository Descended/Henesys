        ...
        for (File file : files) {
            ...
            for (Node infoNode : XMLApi.getAllChildren(mainInfoNode)) {
                ...
                switch (name) {
                    ...
                    default:
                        if (LOG_UNKS) {
                            log.warn("[Reactor] Unhandled info node id {}, {}, value {}", ri.getId(), name, value);
                        }
                }
            }
            addReactorInfo(ri);
...
//            ri.setDrops(DropData.getDropInfoByID(ri.getId()).stream().filter(DropInfo::getReactorDrop).collect(Collectors.toSet())); TODO
            addReactorInfo(ri);
        } catch (IOException e) {
            log.error("Error loading reactor {}", file.getName(), e);
        }
        return ri;
    }
