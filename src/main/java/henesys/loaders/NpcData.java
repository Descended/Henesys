...
//	}
	...
	private static void loadNpcsFromWz() {
		...
		for (File file : Objects.requireNonNull(new File(wzDir).listFiles())) {
			Npc npc = new Npc(0);
			...
			try(DataOutputStream das = new DataOutputStream(new FileOutputStream(file))) {
				...
				npc.getScripts().forEach((key, val) -> {
					try {
						...
						log.error(String.format("Failed to save npc %d", npc.getTemplateId()), e);
					}
				});
			} catch (IOException e) {
				log.error(String.format("Failed to save npc %d", npc.getTemplateId()), e);
	...
	private static Npc loadNpcFromDat(File file) {
		try (DataInputStream dis = new DataInputStream(new FileInputStream(file))) {
			...
			log.error(String.format("Failed to load npc from %s", file.getAbsolutePath()), e);
		}
		return null;
	}
