 ...
 * for important resources used by the henesys.client during encryption,
...
public class NettyClient {
    ...
    private byte[] riv;
    /**
     * Stored length used for net.spirit.ms.connection.packet decryption. This is used for
     * storing the net.spirit.ms.connection.packet length for the next net.spirit.ms.connection.packet that is readable.
     * Since TCP sessions ensure that all data arrives to the server in order,
     * we can decodeByte net.spirit.ms.connection.packet data in the correct order.
     */
    private int storedLength = -1;
    /**
     * Channel object associated with this specific net.spirit.ms.client. Used for all
     ...
     * @param c the channel object associated with this net.spirit.ms.client session.
    ...
    public NettyClient(Channel c, byte[] alpha, byte[] delta) {
        ...
        riv = delta;
        r = new InPacket();
        lock = new ReentrantLock(true); // note: lock is fair to ensure a logical sequence is maintained server-side
    }
    
    /**
     * Gets the InPacket object associated with this NettyClient.
     * @return a net.spirit.ms.connection.packet reader.
     */
    ...
    /**
     * Gets the stored length for the next net.spirit.ms.connection.packet to be read. Used as
     * a decoding state variable to determine when it is ok to proceed with
     * decoding a net.spirit.ms.connection.packet.
     * @return stored length for next net.spirit.ms.connection.packet.
     */
    ...
    /**
     * Sets the stored length for the next net.spirit.ms.connection.packet to be read.
     * @param val length of the next net.spirit.ms.connection.packet to be read.
     */
