    ...
    protected void decode(ChannelHandlerContext chc, ByteBuf in, List<Object> out) {
        ...
        if (c != null) {
            ...
            if (c.getStoredLength() == -1) {
                if (in.readableBytes() >= 4) {
                    ...
                    if (!MapleCrypto.checkPacket(h, iv)) {
                        log.error("[PacketDecoder] | Incorrect packet seq! Dropping client {}.", c.getIP());
                        c.close();
                        return;
                    }
            ...
            if (in.readableBytes() >= c.getStoredLength()) {
                ...
                if (ServerConstants.VERSION < 149) { // Shanda Crypto for versions below 149
                    dec = ShandaCrypto.decryptData(dec);
                }
