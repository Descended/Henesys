...
import static henesys.connection.crypto.BitTools.multiplyBytes;
...
public final class MapleCrypto {
    ...
    public MapleCrypto() {
        cipher = new AES();
        // key is dependent on the version
    ...
    /**
     * Initializes the send and receive version values for henesys.connection.packet
    ...
    /**
     * Creates a header for the new henesys.connection.packet to be sent to the opposite
     * end of the session (Server to Channel for this engine). Contains
     * information about the henesys.connection.packet: whether or not it is valid and the actual
     * length of the henesys.connection.packet.
     *
     * @param delta the input henesys.connection.packet length before adding the header.
     * @param gamma the input sending seed before changing it.
     * @return the header to be sent with this henesys.connection.packet.
     */
    ...
    /**
     * Gets the length of the henesys.connection.packet given the received header.
     *
     * @param delta the henesys.connection.packet header to be used to find the henesys.connection.packet length.
     * @return the length of the received henesys.connection.packet.
     */
    ...
    /**
     * Checks the henesys.connection.packet to make sure it is valid and that the session
     * between the henesys.client and server is secure and legitimate.
     *
     * @param delta the henesys.connection.packet header from the received henesys.connection.packet (4 bytes in length).
     * @param gamma the current receive seed.
     * @return whether or not the henesys.connection.packet is valid (consequently, if not valid,
     * the session is usually terminated).
     */
...
//        System.out.println("Delta[1] ^ gamma[3] = " + Integer.toBinaryString((delta[1] ^ gamma[3]) & 0xFF));
     ...
     * @param gamma current receives seed.
     */
     ...
     * for the next henesys.connection.packet sent or received.
     */
